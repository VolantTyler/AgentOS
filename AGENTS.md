...
- `docs/integrations/agentmail-email-ingest.md` — AgentMail inbound email polling for triage (commodity reporting intake).
...
- `.cursor/skills/requirement-mapping/` — **requirement-mapping** skill: maps every JD bullet to resume/portfolio evidence with direct/adjacent/stretch/gap labels and third-person paste-ready lines; uses local context-portfolio or **https://tylerstahl.dev** in cloud; see `SKILL.md`.
- `.cursor/commands/requirement-map.md` — slash command **`/requirement-map`**; delegates to **requirement-mapper** for application alignment tables (separate from **`/job-fit`**).
- `.cursor/agents/requirement-mapper.md` — **requirement-mapper** subagent; writes `docs/research/requirement-map-*.md`.
...
- `.cursor/skills/email-ingest/` — **email-ingest** skill: AgentMail inbound polling, private digests, `agentos/ingested` labels; see `SKILL.md`.
- `.cursor/commands/email-ingest.md` — slash command **`/email-ingest`**; delegates to **email-ingester**.
- `.cursor/agents/email-ingester.md` — **email-ingester** subagent; polls AgentMail and returns redacted triage.
...
- Subagent definitions in `.cursor/agents/` (including **`events-scout`**, **`ai-news-scout`**, **`email-ingester`**, **`lookahead-networker`**, **`stack-radar`**, **`cos-synthesizer`**, **`feature-evaluator`**, **`feature-testing-agent`**, **`lead-tracker`**, **`job-fit-analyst`**, **`requirement-mapper`**, `onboarding-guide`, `work-strategist`, `research-brief`, `household-coordinator`) are consumed by the Cursor agent runtime — they do not need to be "started" separately.
- Project **skills** live under `.cursor/skills/`; **slash commands** under `.cursor/commands/` (for example **`/events-research`**, **`/lookahead-match`**, **`/weekly-synthesis`**, **`/tech-stack-updates`**, **`/ai-news`**, **`/email-ingest`**, **`/evaluate-feature`**, **`/run-feature-tests`**, **`/lead-tracker`**, **`/job-fit`**, **`/requirement-map`**, and the **job-fit**, **requirement-mapping**, **ai-news-pulse**, and **email-ingest** skills).
- `.env.example` includes `CURSOR_API_KEY` for `@cursor/sdk` (scheduled workflows and local scripts under `scripts/scheduled/`), optional `AGENTMAIL_API_KEY` / `AGENTMAIL_INBOX_ID` for email ingest, plus optional local config for lead-tracker and job-fit-tracker Google Sheet targets.
...
- **Email ingest (local):** `npm run email:list-inboxes` / `npm run email:ingest` — needs `AGENTMAIL_API_KEY` and `AGENTMAIL_INBOX_ID`; see [`docs/integrations/agentmail-email-ingest.md`](docs/integrations/agentmail-email-ingest.md).
...
2. All expected files exist: `AGENTS.md`, `README.md`, `docs/CONTINUITY.md`, `docs/WEEKLY_SYNTHESIS.md`, `docs/RUNTIME_AND_AGENTS.md`, `docs/BOUNDARIES.md`, `docs/integrations/google-sheets-lead-tracker.md`, `docs/integrations/google-sheets-job-fit-tracker.md`, `docs/integrations/agentmail-email-ingest.md`, `docs/integrations/scheduled-tech-stack-radar.md`, `docs/integrations/scheduled-ai-news.md`, `docs/testing/README.md`, `docs/identity-brief.md`, `docs/ONBOARDING_OPEN_QUESTIONS.md`, `.github/workflows/weekly-tech-stack-radar.yml`, `.github/workflows/daily-ai-news.yml`, `.cursor/commands/events-research.md`, `.cursor/commands/lookahead-match.md`, `.cursor/commands/weekly-synthesis.md`, `.cursor/commands/tech-stack-updates.md`, `.cursor/commands/ai-news.md`, `.cursor/commands/email-ingest.md`, `.cursor/commands/evaluate-feature.md`, `.cursor/commands/run-feature-tests.md`, `.cursor/commands/lead-tracker.md`, `.cursor/commands/job-fit.md`, `.cursor/commands/requirement-map.md`, `.cursor/skills/events-research/SKILL.md`, `.cursor/skills/job-fit/SKILL.md`, `.cursor/skills/requirement-mapping/SKILL.md`, `.cursor/skills/lookahead-networker/SKILL.md`, `.cursor/skills/weekly-synthesis/SKILL.md`, `.cursor/skills/tech-stack-pulse/SKILL.md`, `.cursor/skills/ai-news-pulse/SKILL.md`, `.cursor/skills/email-ingest/SKILL.md`, and the `.cursor/agents/*.md` files (including `events-scout.md`, `ai-news-scout.md`, `email-ingester.md`, `lookahead-networker.md`, `cos-synthesizer.md`, `stack-radar.md`, `feature-evaluator.md`, `feature-testing-agent.md`, `lead-tracker.md`, `job-fit-analyst.md`, and `requirement-mapper.md`).
