...
- **job-fit workflow** — [`docs/JOB_FIT_WORKFLOW.md`](docs/JOB_FIT_WORKFLOW.md): durable process for comparing job/company descriptions against Tyler's profile, with duplicate prevention against the tracker sheet, a scorecard-first summary, and durable learning from outcomes over time.
- **Skill:** **job-fit** → [`.cursor/skills/job-fit/SKILL.md`](.cursor/skills/job-fit/SKILL.md): checks for prior evaluations before analysis; surfaces existing date and score when matched.
